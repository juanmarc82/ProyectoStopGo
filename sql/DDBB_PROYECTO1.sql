...
CREATE TABLE Users(
	...
	, Active	BIT NOT NULL
	, CONSTRAINT U_Users_UsersID_NIFNIE UNIQUE(UserID, NIFNIE) );
GO

-- PK assignment in dbo.Users
ALTER TABLE dbo.Users    
ADD CONSTRAINT FK_Users_UserID PRIMARY KEY (UserID);     
GO
...
-- UsersDrivers table Creation dbo.UsersDrivers
IF EXISTS(SELECT 1 FROM sys.tables WHERE [name] = 'UsersDrivers')
BEGIN
    DROP TABLE UsersDrivers;  
    PRINT 'Table already exists, so drop is proceeded';
END
ELSE
BEGIN
...
CREATE TABLE UsersDrivers(
	UserDriverID		INT IDENTITY(1,1) NOT NULL
	, [Name]			VARCHAR (25) NOT NULL
	, Surname			VARCHAR (50) NOT NULL
	, NIFNIE			VARCHAR (9) NOT NULL
	, DateBirth			DATE NOT NULL
	...
	, RegisterDate		DATE NOT NULL
	, Active			BIT NOT NULL
	, CONSTRAINT U_UsersDrivers_UserDriverID_NIFNIE UNIQUE(UserDriverID, NIFNIE) );
GO

-- PK assignment in dbo.UsersDrivers
ALTER TABLE dbo.UsersDrivers    
ADD CONSTRAINT PK_UsersDrivers_UserDriverID PRIMARY KEY (UserDriverID);     
GO

-- FK assignment in dbo.UsersDrivers
ALTER TABLE dbo.UsersDrivers    
ADD CONSTRAINT FK_UsersDrivers_Nationalities_NationalityID FOREIGN KEY (NationalityID)   
    REFERENCES dbo.Nationalities (NationalityID);  
GO
...
CREATE TABLE Vehicles(
	...
	, CONSTRAINT U_Vehicles_VehicleID_LicensePlate_UserDriverID UNIQUE(VehicleID, LicensePlate, UserDriverID) );
GO
...
ALTER TABLE dbo.Vehicles    
ADD CONSTRAINT FK_Vehicles_UsersDrivers_UserDriverID FOREIGN KEY (UserDriverID)   
